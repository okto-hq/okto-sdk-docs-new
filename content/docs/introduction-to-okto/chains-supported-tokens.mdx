---
title: Chains / Tokens
description: Overview of supported chains, wallet structure, testnet availability, and token whitelisting in Okto’s ecosystem.
full: false
---

import SupportedChainCard from "../../../app/components/mdx/SupportedChainCard";
import { FaReact } from "react-icons/fa6";


## Supported Chains

<SupportedChainCard
  chainIcon="/images/polygon-logo.svg"
  chainName="Polygon"
  tokens={[
    { name: "POL", icon: "/images/polygon-logo.svg" },
    { name: "USDT", icon: "/images/usdt-icon.svg" },
    { name: "USDC", icon: "/images/usdc-icon.svg" },
  ]}
  resources={{
    "Solidity": "https://soliditylang.org/",
    "Remix": "https://remix.ethereum.org/",
    "Hardhat": "https://hardhat.org/",
    "OpenZeppelin": "https://www.openzeppelin.com/",
  }}
  supportedTestnet={{
    "Polygon Amoy testnet": "https://amoy.polygonscan.com/",
  }}
/>
<br/>
<SupportedChainCard
  chainIcon="/images/sol-icon.svg"
  chainName="Solana"
  tokens={[
    { name: "SOL", icon: "/images/sol-icon.svg" },
    { name: "USDT", icon: "/images/usdt-icon.svg" },
    { name: "USDC", icon: "/images/usdc-icon.svg" },
  ]}
  resources={{
    "Solana Docs": "https://solana.com/docs",
    "Rust": "https://www.rust-lang.org/",
    "Anchor": "https://www.anchor-lang.com/",
    "Solana Playground": "https://beta.solpg.io/",
  }}
  supportedTestnet={{
    "Solana Devnet": "https://explorer.solana.com/?cluster=devnet",
  }}
/>
<br/>
<SupportedChainCard
  chainIcon="/images/base-icon.svg"
  chainName="Base"
  tokens={[
    { name: "ETH", icon: "/images/eth-icon.svg" },
    { name: "USDC", icon: "/images/usdc-icon.svg" },
  ]}
  resources={{
    "Solidity": "https://soliditylang.org/",
    "Remix": "https://remix.ethereum.org/",
    "Hardhat": "https://hardhat.org/",
    "OpenZeppelin": "https://www.openzeppelin.com/",
  }}
  supportedTestnet={{
    "No Supported Testnet": "#",
  }}
/>
<br/>
<SupportedChainCard
  chainIcon="/images/aptos-icon.svg"
  chainName="Aptos"
  tokens={[
    { name: "APTOS", icon: "/images/aptos-icon.svg" },
    { name: "USDC", icon: "/images/usdc-icon.svg" },
  ]}
  resources={{
    "Aptos Docs": "https://aptos.dev/en",
    "Move": "https://aptos.dev/en/build/smart-contracts",
    "Aptos CLI": "https://aptos.dev/en/build/cli",
    "Aptos SDK": "https://aptos.dev/en/build/sdks",
  }}
  supportedTestnet={{
    "Aptos Testnet": "https://explorer.aptoslabs.com/?network=testnet",
  }}
/>

## Conclusion

Always check the latest information for each chain you are working with, as capabilities and token support may evolve over time. If you have any questions about a specific chain or token, feel free to reach out to our support team.